...
export default function ManageWebcomicScreen({ slug, mangaId }: Props) {
  ...
  useEffect(() => {
    const initializeAndLoad = async () => {
      ...
      if (isLocal && userId) {
        // Para mangas locales, verificar si pertenecen al usuario actual
        try {
          const storageKey = `@mangaty_${userId}_webcomics`;
          const storedStr = await AsyncStorage.getItem(storageKey);
          if (storedStr) {
            const stored = JSON.parse(storedStr);
            const comic = stored.find((c: any) => c.id === mangaId);
            setIsOwner(!!comic);
          } else {
            setIsOwner(false);
          }
        } catch (error) {
          console.error('Error checking manga ownership:', error);
          setIsOwner(false);
        }
      } else {
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        {!isOwner && (
          <View style={styles.readOnlyBanner}>
            <Feather name="lock" size={18} color="#D8708E" />
            <Text style={styles.readOnlyText}>Solo puedes ver este manga. No eres el creador.</Text>
          </View>
        )}
        ...
        {/* Creator Management - solo visible si es propietario */}
        {isOwner && (
          <>
